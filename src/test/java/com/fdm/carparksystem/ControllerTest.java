...
public class ControllerTest {
	...
	public void setDevalultInitialValues() {
		...
		when(iView.getOneTruckSpace()).thenReturn("2");
		when(iView.getCarHourlyRate()).thenReturn("2");
		when(iView.getTruckHourlyRate()).thenReturn("3");
	}
	...
	public void validate_after_input_is_received_and_set_values_of_carpark() throws InvalidPropertiesException {
		...
		verifyViewInputs();
		verify(iValidator).validateInitialValues("20", "2", "2", "3");
		verify(iCarParkOperator).initializeCarPark(20, 2, 2, 3);
	}
	...
	public void never_set_initialize_carpark_properties_if_one_of_input_is_not_valid() throws InvalidPropertiesException {
		...
		doThrow(ipe).when(iValidator).validateInitialValues("-10", "2", "2", "3");
		
		controller.initialize();
		verifyViewInputs();
		verify(iValidator).validateInitialValues("-10", "2", "2", "3");
		verify(iCarParkOperator,never()).initializeCarPark(-10, 2, 2, 3);
	...
	private void verifyViewInputs() {
		...
		verify(iView).getOneTruckSpace();
		verify(iView).getCarHourlyRate();
		verify(iView).getTruckHourlyRate();
	}
